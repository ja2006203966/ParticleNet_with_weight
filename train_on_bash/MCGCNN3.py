...
#     w1 = tf.Variable(initial_value=w, trainable=True, name="a_w1", dtype=tf.float32)
...
#     w2 = tf.Variable(initial_value=w+1, trainable=True, name="a_w2", dtype=tf.float32)
    out2 = _particle_net_base(points, features, mask, setting2,  w=w+2, actype = actype, name='MCGCNN2')
...
#     w3 = tf.Variable(initial_value=w+2, trainable=True, name="a_w3", dtype=tf.float32)
    out3 = _particle_net_base(points, features, mask, setting3,  w=w+3, actype = actype, name='MCGCNN3')
...
#     combined = tf.concat([m1.output, m2.output, m3.output], axis=-1)
    ...
    combined = keras.layers.Conv2D(300, kernel_size=(1,1), strides=1, data_format='channels_last',use_bias=True,
...
def get_particle_net(num_classes, input_shapes,  w=-1, actype = 'max'):
    ...
    features = keras.Input(name='features', shape=input_shapes['features']) if 'features' in input_shapes else None
    mask = keras.Input(name='mask', shape=input_shapes['mask']) if 'mask' in input_shapes else None
    outputs = _particle_net_base(points, features, mask, setting,  w=w, actype = actype, name='ParticleNet')
